   ...
   public class Fourier
    {
        ...
        public void RealFFT(double[] data, bool forward)
        {
            ...
            // checks n is a power of 2 in 2's complement format       
            Console.Write("data length is " + n);                              
            if ((n & (n - 1)) != 0)
                throw new ArgumentException(
                    "data length " + n + " in FFT is not a power of 2"
                    );
        ...
        {
            int i = 0;
            double[] arra = new double[500];
            string[] lines = System.IO.File.ReadAllLines(@"Cos_20Hz_Sin_7Hz_128.txt");
            foreach (string line in lines)
            {
                // Use a tab to indent each line of the file.
                Console.WriteLine("\t" + line);
              //  i++;
                arra[i] = Convert.ToDouble(line);
                i++;
            }
            Console.WriteLine("total elements "+ i + "\n");
            int num=i;
            double[] tested = new double[num];

            for (int j = 0; j < i;j++ )
            {
                tested[j] = arra[j];
                Console.WriteLine(tested[j]+"\n");
            }
          ...
          //  arra[0]=0.0;
            ...
            t.TableFFT(tested, true);
            int count = 0;
            double temp = 0;
            double record=0;
            double len = Convert.ToDouble(i);
            Console.WriteLine(len);
            for (int k = 0; k < i; k++)
            {
                count++;
              //  Console.WriteLine("index " + k +"  updated value " + tested[k] + "   number count "+ count +"\n" );
                if (tested[k] >= temp)
                {
                    if (k != 0)
                    {
                        temp = tested[k];
                      //  Console.WriteLine("largest num us " + temp);
            ...
            Console.WriteLine("the length of the array " + tested.Length);
            Console.WriteLine("hightest value in the array " + temp + "the index is " + record );
            Console.ReadKey();
        }
